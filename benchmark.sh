...
function compile() {
    rm -f benchmark/ssvm/compile.time benchmark/lucet/compile.time benchmark/wavm/compile.time benchmark/wasmer_*/compile.time benchmark/wamr/compile.time
    for ((i=0; i<"${#NAME[@]}"; ++i)); do
        ...
        (time "$WAMRC" -o benchmark/wamr/"${NAME[i]}".so build/"$MODE"/"${NAME[i]}".wasm 2>&1) 2>> benchmark/wamr/compile.time || true
    done
}
...
function benchmark_wamr() {
    echo benchmark_wamr
    for ((i=0; i<"${#NAME[@]}"; ++i)); do
        LOG="benchmark/wamr/"${NAME[i]}".log"
        rm -f "$LOG"
        touch "$LOG"
        for ((j=0; j<$COUNT; ++j)); do
            time "$WAMR" benchmark/wamr/"${NAME[i]}".so "${ARGS[i]}" <benchmark/random >&/dev/null
        done 2> "$LOG"
        /usr/bin/time -o "benchmark/wamr/"${NAME[i]}".time" --verbose "$WAMR" benchmark/wamr/"${NAME[i]}".so "${ARGS[i]}" <benchmark/random >&/dev/null
    done
}
...
function print_result() {
    ...
    echo
    # for type in native ssvm wamr lucet wavm wasmer_singlepass wasmer_cranelift wasmer_llvm wasmer_jit v8 docker; do
    for type in native ssvm wamr docker; do
        echo -n "$type"
        for name in "${NAME[@]}"; do
